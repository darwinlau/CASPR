    ...
    methods
        ...
        function intervals =  evaluateFunction(obj, model, ws_ray)
            ...
            if is_dof_translation
                u_ind = find((OB_i_end(i,:)-OB_i_begin(i,:)));
                for  i = 1:size(OB_i_begin,1)
                    cable_triangle = [OA_i_hat(i,:);OB_i_begin(i,:);OB_i_end(i,:)];
                    cable_surface = triangulation([1 2 3],cable_triangle);
                    obstacle_surface.Points = obj.vertices;
                    obstacle_surface.ConnectivityList = obj.connectivity;
                    [~, intSurface] =  SurfaceIntersection(cable_surface, obstacle_surface);
                    P_j = intSurface.vertices;
                    %                    scatter3(intSurface.vertices(:,1),intSurface.vertices(:,2),intSurface.vertices(:,3),'filled'  )
                    
                    for j = 1:size(P_j,1)
                        P_j_on_OB_i = obj.LineIntersection3D([OA_i_hat(i,:);OB_i_begin(i,:)],[P_j(j,:);OB_i_end(i,:)]);
                        q_u = [q_u,P_j_on_OB_i(u_ind)];
%                         drawline_x = [P_j_on_OB_i(1,1),OA_i_hat(i,1)];
%                         drawline_y = [P_j_on_OB_i(1,2),OA_i_hat(i,2)];
%                         drawline_z = [P_j_on_OB_i(1,3),OA_i_hat(i,3)];
%                         line(drawline_x,drawline_y,drawline_z,'Color','b');
                    end
                ...
                q_sample = linspace(q_begin(free_variable_index),q_end(free_variable_index),obj.MAX_DEGREE_ORIENTATION+1);
                ...
                for i = 1:size(q_sample,2)
                    ...
                    u_sample_per_cable = [];
                    triangle_edges =  unique(sort([obj.connectivity(:,[1 2]);obj.connectivity(:,[2 3]);obj.connectivity(:,[1 3])],2),'rows');
                    for j = 1:size(triangle_edges,1)
                        OV_i = obj.vertices(triangle_edges(j,1),:);
                        OE_i = [obj.vertices(triangle_edges(j,2),:) - obj.vertices(triangle_edges(j,1),:)];
                        for ii = 1:size(OB_i_u,1)
                            u_sample_per_cable = [u_sample_per_cable;...
                                (1+u_sample(i)^2)*cross(OB_i_u(ii,:)-OA_i_hat(ii,:),OE_i)*(OV_i - OA_i_hat(ii,:))'];
                        end
                    end
                    
                    u_sample_per_pose = [u_sample_per_pose,u_sample_per_cable];
                end
                for i = 1:size(u_sample_per_pose,1)
                    u_coeff = GeneralMathOperations.PolynomialFit(u_sample', u_sample_per_pose(i, :)', obj.MAX_DEGREE_ORIENTATION);
                    u_value = roots(round(u_coeff,obj.ROUNDING_DIGIT))';
                    q_u = [q_u,2*atan(u_value)];
                end
            end
            q_u = q_u(imag(q_u)==0);
            q_u = unique(q_u);
            q_u(q_u < q_begin(free_variable_index)) = [];q_u(q_u > q_end(free_variable_index)) = [];
            if ~isempty(q_u)
                q_intersected = repmat(q_begin,1,size(q_u,2));
                q_intersected(free_variable_index,:) = q_u;
            ...
            [tri_u,tri_v,~] = obj.RayTriangleEquation();
            for i = 1:size(q_intersected,2) - 1
                q_test = (q_intersected(:,i)+q_intersected(:,i+1))/2;
                ...
                length = vecnorm(rOAi_all(1:3,:)-rOAi_all(4:6,:));
                OB_i= rOAi_all(1:3,:);
                OA_i= rOAi_all(4:6,:);
%                 tic
%                 for j = 1:size(OB_i,2)
%                     O = OA_i(:,j);
%                     D =  (OB_i(:,j) - OA_i(:,j)) /norm((OB_i(:,j) - OA_i(:,j)));
%                     for k = 1:size(obj.connectivity,1)
%                 [flag1(j,k), u1(j,k), v1(j,k), t1(j,k)] = obj.SegmentTriangleIntersection (O', D', obj.vertices(obj.connectivity(k,:),:));
%                     end
%                 end
%                 toc1 = toc;
                Si_unit_all =   (OA_i - OB_i)./length;
%                 tic
                [v1,v2,v3,v4,v5,v6,v7,v8,v9,o1,o2,o3,d1,d2,d3] = obj.VarSubs(OA_i', Si_unit_all', vertices_i);
...
%                     intersected
    ...
    methods (Access = private)
        ...
        %% functio to find 3D line intersection
