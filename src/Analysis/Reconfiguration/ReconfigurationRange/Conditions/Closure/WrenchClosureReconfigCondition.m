...
classdef WrenchClosureReconfigCondition < ReconfigConditionBase
    ...
    methods (Access = private)
        function intervals = evaluate_function_fully_restrained(obj,reconfig_element)
            ...
            maximum_degree = 1; % tmp for linear rail
...
%                         if numel(unique(sign(sign_vector))) == 1
                            % Add the interval
        ...
        function intervals = evaluate_function_redundantly_restrained(obj,reconfig_element)
            ...
            for cable_id = 1:obj.model.numCables
                ...
                if norm(reconfig_element.OA_begin(:,cable_id) - reconfig_element.OA_end(:,cable_id)) ~= 0
                    ...
                    polynomial_coefficients_null = zeros(obj.model.numDofs+1,maximum_degree+1); % Initialised once it is always completely updated
                    sign_vector = zeros(obj.model.numDofs+1,1);
                    null_roots = [0;1];
                    for combination_index = 1:number_combinations
                        if(leading_zero_number(combination_index) ~= -1)
                            % Repeat for each combination and k
...
%                                 end
                    ...
                    for root_num = 1:size(null_roots,1)-1
                         mean_root = 0.5*(null_roots(root_num) + null_roots(root_num+1));   
                         OA = OA_0;
                         OA(:,cable_id) = reconfig_element.OA_begin(:,cable_id) + mean_root*(reconfig_element.OA_end(:,cable_id) -  reconfig_element.OA_begin(:,cable_id));
                        
                        % Update the model and Obtain the Jacobian for computation
                        [obj.model,A] = obj.updateModelOA(obj.model,OA,q);
                        [~,situtaion] = obj.WCW_evaluation(obj.model);
                        [obj.model,A] = obj.updateModelOA(obj.model,OA_0,q);
                        if situtaion == 1
                         new_interval = [null_roots(root_num), null_roots(root_num+1)];
                         single_cable_interval = obj.set_union(single_cable_interval,new_interval);
                        end
    ...
    methods (Static)
        ...
        function [TF,situtaion] = WCW_evaluation(model)
...
%             row_C = combnk(1:size(model.L,1),size(model.L,2)+1);
            ...
            [n,m] = size(model.L');
            H = 2*eye(m); f =ones(1,m);
            A_ieq = [];b_ieq = [];
            A_eq = L;
            b_eq = zeros(n,1);
            ub = 10000000*ones(m,1);     lb = 0.00001*ones(m,1);
            options = optimoptions('linprog','Display','none');
            [cable_force,~,flag,~] = linprog(f',A_ieq,b_ieq,A_eq,b_eq,lb,ub,options);
            if flag == 1
                cable_force = round(cable_force,6);
                cable_force(cable_force==0) = [];
                TF = min(cable_force)/max(cable_force);
            else
                TF = 0;
            end
            if TF  ~= 0
