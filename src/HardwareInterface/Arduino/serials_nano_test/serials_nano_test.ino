...
void loop() {
  ...
  if ((millis() - t_ref) > TIME_STEP * 1000) {
    ...
    if (newCommand) {
      interpolation = 1;
      sendCommandCounter = 2;
      pwmCommandMiddle = ((pwmCommand + lastPWM) / 2.0) + 0.5; //finding middle value, rounding
      detectCrossing();
      if (cross == true) {
        crossing();
      }
      newCommand = 0;
      ctrl_motor(pwmCommandMiddle);
    } else if (interpolation) {
      ctrl_motor(pwmCommand);
      interpolation = 0;
    } else if (sendCommandCounter > 0) {
      ctrl_motor(pwmCommand); //transmits the output signal towards the motor
      sendCommandCounter--;
    }
    
    //sendFeedbackPWM();
 /*   sinCounter++;
    sinRad = sinCounter * 0.0125;
    servoPulse(MOTOR_PIN, (977 + 480*sin(sinRad)));    */
  }
}
...
void readSerial() //receive characterizing prefix (+ length in 2 digit Hex, with manipulation of first bit for sign)
{
  if (Serial.available() > 0) {
    ...
    else if (command == RECEIVE_PWM_REQUEST) { //o
      sendFeedbackPWM();
    }
...
void readAngularChange() {
  char tmp[2];
  tmp[0] = strReceived[LENGTH_HEX_ANGLE * NANO_ID + 1];
  tmp[1] = strReceived[LENGTH_HEX_ANGLE * NANO_ID + 2];
  tmp[2] = '\0';
...
void sendFeedback() {
  ...
  */
  ...
  char sendingFeedback[LENGTH_HEX_ANGLE + 1];
